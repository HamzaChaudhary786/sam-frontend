...
const StationViewModal = ({ isOpen, onClose, station, onEdit }) => {
    ...
    const [activeTab, setActiveTab] = useState("stationView");
    const [assignmentHistory, setAssignmentHistory] = useState([]);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);

    // Fetch station assignment history when the modal is open and station ID is available
    useEffect(() => {
        if (isOpen && station?._id && activeTab === "assignmentHistory") {
            const fetchAssignmentHistory = async () => {
                setLoading(true);
                try {
                    const response = await axios.get(`${BACKEND_URL}/station-history/${station._id}`);
                    // Set assignmentHistory to response.data.data, as the data is nested
    ...
    const latestAssignmentId = assignmentHistory.reduce((latest, item) => {
        if (!latest) return item._id;
        return new Date(item.fromDate) > new Date(
            assignmentHistory.find(a => a._id === latest)?.fromDate
        )
            ? item._id
            : latest;
    }, null);
                ...
                <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="px-6 pt-4">
                        <div className="flex border-b border-gray-200">
                            <button
                                className={`px-4 py-2 font-medium text-sm ${activeTab === "stationView"
                                    ? "border-b-2 border-blue-600 text-blue-600"
                                    : "text-gray-500 hover:text-gray-700"
                                    }`}
                                onClick={() => setActiveTab("stationView")}
                            >
                                Station View
                            </button>
                            <button
                                className={`px-4 py-2 font-medium text-sm ${activeTab === "assignmentHistory"
                                    ? "border-b-2 border-blue-600 text-blue-600"
                                    : "text-gray-500 hover:text-gray-700"
                                    }`}
                                            ...
                                            <tbody>
                                                {assignmentHistory?.map((assignment, index) => (
                                                    <tr
                                                        ...
                                                        key={index}
                                                        className={`border-t border-gray-200 ${assignment._id === latestAssignmentId ? "bg-yellow-50" : ""}`}
                                                    >
