...
const BulkAssetAssignment = () => {
  ...
  try {
    ...
    if (response.data && response.data.success) {
      ...
      } else if (response.data.data && response.data.data.asset && Array.isArray(response.data.data.asset)) {
  ...
  const searchAssets = (query, rowId) => {
    console.log("Searching assets:", { query, rowId, mallkhanaAssets: mallkhanaAssets.length });
    ...
    const filteredAssets = mallkhanaAssets.filter(asset => {
      ...
      return (
        ...
        (asset.weaponNumber && asset.weaponNumber.toLowerCase().includes(searchTerm)) ||
        (asset.pistolNumber && asset.pistolNumber.toLowerCase().includes(searchTerm)) ||
        (asset.vehicleNumber && asset.vehicleNumber.toLowerCase().includes(searchTerm))
      );
    });
    ...
    try {
      ...
      if (result.success) {
        ...
        const filteredStations = stations.filter(station =>
          station.name && station.name.toLowerCase().includes(query.toLowerCase())
        );
  ...
  const clearAsset = (rowId) => {
    setAssignmentRows(prev => prev.map(row =>
      row.id === rowId ? { ...row, asset: null } : row
    ));
  };

  const clearEmployee = (rowId) => {
    setAssignmentRows(prev => prev.map(row =>
      row.id === rowId ? { ...row, employee: null } : row
    ));
  };

  const clearStation = (rowId) => {
    setAssignmentRows(prev => prev.map(row =>
      row.id === rowId ? { ...row, station: null } : row
    ));
  };
  ...
  const validateForm = () => {
    ...
    assignmentRows.forEach((row, index) => {
      ...
      if (!row.employee && !row.station) {
        errors.push(`Row ${index + 1}: At least one of Employee or Station must be selected`);
      }
...
const handleSaveAll = async () => {
  ...
  try {
    ...
    if (response.data && response.data.success) {
      toast.success(`Successfully created ${assignments.length} asset assignments!`);
      
      // Show success message and navigate
      setTimeout(() => {
        if (window.confirm("Assets assigned successfully! Would you like to create more assignments?")) {
          // Reset form
          setHeaderData({
            receiveDate: new Date().toISOString().split('T')[0],
            referenceNumber: '',
      ...
      toast.error(`Failed to create asset assignments: ${response.data?.message || 'Unknown error'}`);
    }
  } catch (error) {
    console.error("Bulk asset assignment error:", error);
    const errorMessage = error.response?.data?.message || error.message || "Failed to create asset assignments";
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Bulk Asset Assignment</h1>
                ...
                <tr key={row.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 relative">
                    {row.asset ? (
                      <div className="flex items-center">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-gray-500 truncate">
                            Type: {row.asset.type || 'N/A'} | Category: {row.asset.category || 'N/A'}
                          </div>
                          {(row.asset.weaponNumber || row.asset.pistolNumber || row.asset.vehicleNumber) && (
                            <div className="text-xs text-gray-500 truncate">
                              {row.asset.weaponNumber || row.asset.pistolNumber || row.asset.vehicleNumber}
                            </div>
                          )}
                      ...
                      <div className="relative">
                        <input
                          type="text"
                          placeholder={!headerData.mallkhana ? "Select Mallkhana first..." : "Search asset..."}
                          ...
                          onChange={(e) => {
                            ...
                            setAssetSearch(prev => ({ ...prev, [row.id]: value }));
                          ...
                          <div className="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
                            {searchResults.assets[row.id].map((asset) => (
                              ...
                              >
                                ...
                                {(asset.weaponNumber || asset.pistolNumber || asset.vehicleNumber) && (
                    ...
                    {row.employee ? (
                      <div className="flex items-center">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-gray-500 truncate">
                            {row.employee.personalNumber || row.employee.pnumber}
                          </div>
                        </div>
                        <button
                      ...
                      <div className="relative">
                        <input
                          ...
                          onChange={(e) => {
                            const value = e.target.value;
                            setEmployeeSearch(prev => ({ ...prev, [row.id]: value }));
                    ...
                    {row.station ? (
                      ...
                      <div className="relative">
                        <input
                          ...
                          onChange={(e) => {
                            const value = e.target.value;
                            setStationSearch(prev => ({ ...prev, [row.id]: value }));
                  ...
                  <td className="px-6 py-4">
                    <input
                      ...
                      disabled={loading}
                      onChange={(e) => handleAssignmentChange(row.id, "assignmentDate", e.target.value)}
                  ...
                  <td className="px-6 py-4">
                    <textarea
                      ...
                      value={row.remarks}
                      disabled={loading}
                      onChange={(e) => handleAssignmentChange(row.id, "remarks", e.target.value)}
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-yellow-700">
                  <p>Please select a Mallkhana in the header section above to load available assets for assignment.</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-blue-700">
                  <p>The selected Mallkhana "{headerData.mallkhana.name}" does not have any assets available for assignment.</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t shadow-md p-4 flex gap-3 justify-end z-50">
        ...
        <button
          ...
          disabled={loading || assignmentRows.length === 0 || !headerData.mallkhana}
