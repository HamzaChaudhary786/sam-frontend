...
const DrillDistrictPage = ({ 
  ...
  const fetchDistrictData = async () => {
    ...
    try {
      ...
      if (tehsil) {
        const tehsilURL = `${BACKEND_URL}/tehsil-to-district?tehsil=${tehsil}&district=${district}&page=1&limit=10`;
        ...
        // Use district-specific API (you might need to create this endpoint)
        // Option 1: If you have a district-specific endpoint
        const districtURL = `${BACKEND_URL}/district/${district}?page=1&limit=10`;
  ...
  const getTehsilData = () => {
    if (data?.level === "district" && data?.tehsils) {
      // tehsil-to-district API response
      return data.tehsils;
  ...
  const getSummaryData = () => {
    if (data?.level === "district" && data?.districtInfo) {
      ...
      return {
        ...
        activeStations: data.districtInfo.totalStations // Assuming all are active
      };
    } else if (data?.districts) {
                ...
                <div className="flex items-center">
                  <div className="w-20 bg-gray-200 rounded-full h-2 mr-2">
                    <div
                      ...
                      style={{
                        width: `${
                          (count / Math.max(...Object.values(data.employeeStats.byDesignation))) * 100
                        }%`,
                      }}
                    ></div>
                  </div>
                ...
                <div className="flex items-center">
                  <div className="w-20 bg-gray-200 rounded-full h-2 mr-2">
                    <div
                      ...
                      style={{
                        width: `${
                          (count / Math.max(...Object.values(data.employeeStats.byGrade))) * 100
                        }%`,
                      }}
                    ></div>
                  </div>
